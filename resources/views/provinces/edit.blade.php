...
@section('content')
                  ...
                  <form method="POST" action="{{ url($province->path()) }}">
		    {{ csrf_field() }}
		    {{ method_field('PATCH') }}
	      <div class="card">
		<div class="card-header">
		  <h5 class="card-title mb-0">Edit Province</h5>
		</div>
		<div class="card-body">
		    <div class="form-group">
		        ...
		        @php
			    $v = old('country') ?? $province->country_id;
			@endphp
		        <option></option>
			...
			@endforeach
		      ...
		      @endif
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control{{ $errors->first('code') ? ' is-invalid' : '' }}" id="code" name="code" value="{{ old('code') ?? $province->code}}" />
		    ...
		    <div class="form-group">
                      ...
                      <input type="text" class="form-control{{ $errors->first('name') ? ' is-invalid' : '' }}" id="name" name="name" value="{{ old('name') ?? $province->name }}" />
		      ...
		      <textarea class="form-control{{ $errors->first('description') ? ' is-invalid' : '' }}" id="description" name="description" rows="3">{{ old('description') ?? $province->description}}</textarea>
